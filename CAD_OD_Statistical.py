...
for yrs in thresholds_yrs:
    ...
    if not df_safe.empty:
        ...
        raw_importances = best_xgb.feature_importances_
        all_features = np.array(X_train.columns)

        features_to_hide = ['Master_Stage', 'Age_Stage_Interaction', 'Age_Tumor_Size_Interaction']
        keep_mask = ~np.isin(all_features, features_to_hide)
        
        filtered_features = all_features[keep_mask]
        filtered_importances = raw_importances[keep_mask]
        
        sorted_idx = np.argsort(filtered_importances)
        features_sorted = filtered_features[sorted_idx]
        importances_sorted = filtered_importances[sorted_idx]
        ...
        for f in features_sorted:
            if f in rename_dict:
                clean_labels.append(rename_dict[f])
            else:
                clean_labels.append(f.replace('_', ' '))
                
        features_sorted = np.array(clean_labels)
        colors = plt.cm.magma(np.linspace(0.1, 0.8, len(features_sorted)))

        plt.figure(figsize=(10, 6))
        plt.barh(range(len(features_sorted)), importances_sorted, align='center', color=colors)
        plt.yticks(range(len(features_sorted)), features_sorted, fontsize=11)
        plt.title(f'XGBoost Feature Importance ({yrs}-Year Model)', fontsize=14, fontweight='bold')
        ...
        # --- FIGURE 2: KAPLAN-MEIER SURVIVAL VALIDATION ---
        ...
        mask_ind = (df_eval['Predicted_Class'] == 1)

        if sum(mask_agg) > 0 and sum(mask_ind) > 0:
            kmf_agg.fit(T[mask_agg], event_observed=E[mask_agg], label=f"Predicted Aggressive ({yrs}-Year Model)")
            kmf_ind.fit(T[mask_ind], event_observed=E[mask_ind], label=f"Predicted Indolent ({yrs}-Year Model)")

            results = logrank_test(T[mask_agg], T[mask_ind], event_observed_A=E[mask_agg], event_observed_B=E[mask_ind])

            plt.figure(figsize=(10, 6))
            kmf_ind.plot_survival_function(color='blue', linewidth=2.5, ci_show=False)
            kmf_agg.plot_survival_function(color='red', linewidth=2.5, ci_show=False)
            ...
            print(f"[!] Not enough class variance to plot Kaplan-Meier for {yrs}-Year threshold.")
